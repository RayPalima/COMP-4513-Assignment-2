...
function Browse({ products }) {
  ...
  const [sort, setSort] = useState("name"); // default: name
  ...
  const removeTag = (tag) => {
    ...
    setFilters((prev) => ({
      ...
      sizes: prev.sizes.filter((s) => s !== tag),
      colors: prev.colors.filter((clr) => clr !== tag)
    }));
  };
  ...
  const activeTags = [
    filters.gender && (filters.gender === "mens" ? "Men" : "Women"),
    ...filters.categories,
    ...filters.sizes,
    ...filters.colors
  ].filter(Boolean);
